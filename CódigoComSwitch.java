...
class Metodos {
            ...
            if (p != null) {
                ...
                if (r.next()){
                    ...
                    String AlinhamentoSQL = "SELECT music_name FROM musica WHERE id_musica = " +
                            ...
                            "(SELECT id_concerto FROM concerto where local = ? AND data = ?))"; //+
                            //"ORDER BY id_alinhamento"; //NAO SEI SE ESTA ULTIMA PARTE VAI FUNCIONAR BEM
    ...
    void executa_alteraalinhaconcerto (String user, String local, String data, String music_name, String nova_posicao, Connection connection) throws  SQLException{
        ...
        if (r.next()){
            ...
            String checkPosicaoSQL = "SELECT posicao_no_alinhamento FROM alinhamento WHERE id_musica = " +
                    "(SELECT id_musica FROM musica WHERE music_name = ?)";
            PreparedStatement ps = connection.prepareStatement(checkPosicaoSQL);
            ps.setString(1, music_name);
            ResultSet rs = ps.executeQuery();
            int pos = 0;
            if (rs.next()){
                ...
                //String UpdateNovaSQL = "UPDATE alinhamento SET posicao_no_alinhamento = nova_posicao WHERE music_name = ?";


                String UpdateAntigaSQL = "UPDATE alinhamento SET posicao_no_alinhamento = pos WHERE music_name = ?";
            ...
            if (opcao == 1){
                ...
                switch (opcao){
                    ...
                    case 6:
                        ...
                        m.executa_search_info(search, artist_name, connection);
                        break;
                    ...
                    case 8:
                        ...
                        m.executa_search_info_concerto(local, data, connection); //FALTA TESTAR
                        break;
                    case 9:
                        //Share music
                        //ESTA É TUA TOINO
